...
export default {
  ...
  methods: {
        ...
        .then(([Map, Basemap, MapImageLayer, MapView]) => {
          var basemap = new Basemap({
            baseLayers: [
              new MapImageLayer({
                url:
                  "http://192.168.140.147:6080/arcgis/rest/services/office/MapServer",
                // "http://http://192.168.140.147/:6080/arcgis/rest/services/yingqu/MapServer",
                title: "Basemap"
              })
            ],
            title: "basemap",
    ...
    getMapData() {
      let that = this;
      let dataArr = this.barracksMapData;
      dataArr.map(value => {
        console.log(value);
        that.markMap(value);
      });
      that.markMap({
        ...
        resId: 20,
        equipmentName: "开窗",
        data: [121.55421604354626, 31.33798484212217]
      });
    },
