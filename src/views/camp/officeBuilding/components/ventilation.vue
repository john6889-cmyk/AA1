    ...
    <outSidebox>
      <div slot="content" class="ventilationContent">
        <div class="contentHearder">
          <span class="hearder-title">通风设备管理</span>
          <el-select
            v-model="selectDevide"
            placeholder="请选择"
            size="mini"
            style="width:40%"
            v-if="windowOpenerOptions.length > 0"
          >
            <el-option
              v-for="item in windowOpenerOptions"
              :key="item.resId"
              :value="item.resId"
          ...
          <ul v-if="windowOpenerOptions.length > 0">
            <li>
              <i class="icon iconfont">&#xe6a7;</i>名称：<span>
                {{ selVentilationData.name || "-" }}
              </span>
            </li>
            <li>
              <i class="icon iconfont">&#xe671;</i>品牌：<span>
                {{ selVentilationData.vendor || "-" }}
              </span>
            </li>
            <li>
            ...
            <li>
              ...
              <span
                v-if="
                  selVentilationData &&
                    selVentilationData.property &&
                    selVentilationData.property.status == 'open'
                "
              >
                开</span
              >
